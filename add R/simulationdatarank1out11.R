...
axisminy <- -1
axismaxy <- 1.2
savefigure <- "simulationrank1out11casym"
...
### The
### Warning message: In reconca(dt, ncp = 0, initials = 1, threshold = 1e-15, outliercell = outliercell,  :
###             Stopped after  1e+05  iterations
###is due to the sum((diff^2)) = 0, such that old is 0. We can add the following code to the reconca.R:
###if (is.nan(criterion)) {
###  if ((sum((Xhat[missing] - recon[missing])^2) < threshold) && (nb.iter > 50)) {
###    nb.iter <- 0
###    print(paste("Stopped because objective is the same as the reconstitution value except outliers and previous imputed value is similar the current value"))
###  }
###}
...
X[which(rownames(dt) == "1"), which(colnames(dt) == "a")]
